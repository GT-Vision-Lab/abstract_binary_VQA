function feat = ExtractFeaturesForEachImage_im(scene,pri,sec, catlist, inslist, ...
...
%         if strcmp(avobj{ii}.instance{1}.name, scene.sec_name)
    ...
    if scene.primaryName(1) == ' ' || scene.secondaryName(1) == ' '
%     if isempty(pri) || isempty(sec)
        ...
        elseif scene.primaryName(1) ~= ' ' && scene.secondaryName(1) == ' '
            priins = pri.ins+1;
